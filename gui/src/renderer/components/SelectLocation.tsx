...
export default class SelectLocation extends Component<IProps> {
  ...
  private heightAdditionRef = React.createRef<HeightAddition>();
            ...
            <NavigationContainer>
              ...
              <View style={styles.container}>
                <NavigationScrollbars ref={this.scrollView}>
                  <HeightAddition ref={this.heightAdditionRef}>
                    <View style={styles.content}>
                      {this.props.locationScope === LocationScope.relay ? (
                        <ExitLocations
                          ...
                          onWillCollapse={this.heightAdditionRef.current?.resetHeight}
                        />
                      ) : (
                        <BridgeLocations
  ...
  private onWillExpand = (buttonDimensions: DOMRect, additionalHeight: number) => {
    buttonDimensions.height += additionalHeight;
    this.heightAdditionRef.current?.addHeight(additionalHeight);
    this.scrollView.current?.scrollIntoView(buttonDimensions);
  };
}
...
class HeightAddition extends Component<IHeightAdditionProps> {
  ref = React.createRef<HTMLDivElement>();

  public addHeight(additionalHeight: number) {
    this.resetHeight();
    if (this.ref.current) {
